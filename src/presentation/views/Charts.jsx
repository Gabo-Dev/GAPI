...
export function Charts() {
	...
	const { detailData, loading, error, fetchCryptoDetails, dataSource } =
		useCryptoContext();
	...
	const viewWrapperStyles =
		"flex-1 flex flex-col w-full min-h-[450px] md:min-h-[500px]";
					...
					<div className="flex bg-slate-200/50 dark:bg-slate-800/50 p-1 rounded-xl w-full md:w-fit">
						{ASSET_METADATA.map((coin) => (
							<button
								...
								className={`flex-1 md:flex-none px-4 py-2 md:px-6 rounded-lg text-xs md:text-sm font-black tracking-widest transition-all duration-300 ${
					...
					<div className="flex bg-slate-200/50 dark:bg-slate-800/50 p-1 rounded-xl w-full md:w-fit">
						{ALLOWED_PERIODS.map((period) => (
							<button
								...
								className={`flex-1 md:flex-none px-3 py-2 md:px-4 rounded-lg text-[10px] md:text-xs font-bold uppercase transition-all duration-300 ${
									activePeriod === period
				...
				<div className="flex-1 w-full  relative z-10">
					{loading ? (
						<div className="absolute inset-0 flex items-center justify-center">
							<span className="animate-pulse font-bold text-slate-400 tracking-widest text-lg md:text-xl">
								ANALYZING TREND...
							</span>
						</div>
					) : error ? (
						<div className="absolute inset-0 flex items-center justify-center">
							<div className="p-4 bg-red-500/10 border border-red-500/20 text-red-500 rounded-2xl font-bold">
								{error}
							</div>
						</div>
					) : (
