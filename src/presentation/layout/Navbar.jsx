...
export function Navbar() {
	...
	const navigationRoutes = [
		...
		{ path: "/about", label: "About GAPI" },
	];

	const desktopWrapperStyles =
		"hidden md:block fixed bottom-2 left-1/2 -translate-x-1/2 z-50";
	const desktopListStyles =
		"flex items-center gap-10 px-8 py-3.5 rounded-full bg-white/70 dark:bg-slate-800/70 backdrop-blur-lg border border-slate-200/50 dark:border-slate-700/50 shadow-xl";
	const desktopLinkStyles =
		"text-sm font-semibold tracking-wide hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-300";

	const mobileWrapperStyles =
		"md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-end justify-center w-full px-4";
					...
					{navigationRoutes.map(({ path, label }) => (
						<li key={path}>
							<Link
								...
								className={`${desktopLinkStyles} ${location.pathname === path ? "text-blue-600 dark:text-blue-400 font-extrabold" : "text-slate-600 dark:text-slate-300"}`}
							>
								{label}
							</Link>
						</li>
							...
							<div className="flex flex-row justify-around flex-1 mr-2">
								{navigationRoutes.map(({ path, label }) => (
									<Link
										...
										to={path}
										onClick={() => setIsExpanded(false)}
										className={`text-xs font-extrabold active:text-blue-500 ${location.pathname === path ? "text-blue-600 dark:text-blue-400" : "text-slate-800 dark:text-slate-100"}`}
									>
										{label.toUpperCase()}
									</Link>
								))}
